...
# Generate SSL certificate and key using OpenSSL command-line tool
...
# Define public IP address and port
IP_address = '192.168.162.114'  # Use your public IP address here
    ...
    if len(keypress[0]) > 0:
        ...
        if len(answering) > 0:
            if time.time() - t0 >= 10:
                ...
                # send_to_all(server, str(participants[mapping[who_buzzed]]) + " -0.5") # Removed deduction of 0.5 points
                # print(str(participants[mapping[who_buzzed]]) + " -0.5") # Removed print for deduction
                update_marks(who_buzzed, 0)  # No deduction for wrong answer
                time.sleep(3)
                quiz()
            else:
                time.sleep(3)
            ...
            if socket == Person[0]:
                try:
                    ...
                    if ans == answer[0]:
                        ...
                        update_marks(socket, 0)  # No deduction for wrong answer
                        Person[0] = server
                except ValueError:
